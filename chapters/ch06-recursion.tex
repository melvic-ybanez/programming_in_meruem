...
But there is one problem --- Meruem doesn't support \keyword{tail-call optimization} --- a method used by a compiler or interpreter to avoid allocating a new stack frame for a function during a \keyword{tail call}. So, if we execute \code{(tail-sum1ton 400 0)}, we'll still get a \code{StackOverflow} error.

\subsection{\texttt{tail-rec} and \texttt{recur}}
The standard way to solve the stack overflow issue (that we saw earlier) in Meruem is to use the \code{tail-rec} and \code{recur} functions.

The main changes that we did is the calling of the \code{tail-rec} and \code{recur} functions. \code{tail-rec} takes two arguments. The first one is a list of pairs. 
