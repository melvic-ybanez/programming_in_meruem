...
Lisps are quite known for their support for metaprogramming using \keyword{macros}, you're probably expecting to learn macros in this book. Sadly, that's not going to happen. This book would focus only on the basic functional aspects of Meruem. 
...
Save it as \keyword{hello\_world.mer}. Then, \code{cd} into where you saved \keyword{hello\_world.mer} and run the following command: \code{java -jar meruem.jar hello\_world}. You should be able to see the following output:
...
The \code{hello\_world} is the name of the source file you want to run. A Meruem source file ends with a \keyword{.mer}. Basically, if the source file is not provided, Meruem will fire up the REPL for you. 
